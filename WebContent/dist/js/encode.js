...
//区位码中01-09区是符号、数字区，16-87区是汉字区，10-15和88-94是未定义的空白区。所以编码汉字的话需要从16区开始，即区号和位号上分别加上0xa0就可以定位到汉字区。
//**16-87区
var gbData = [
				...
				'住注祝驻抓爪拽专砖转撰赚篆桩庄装妆撞壮状椎锥追赘坠缀谆准捉拙卓桌琢茁酌啄着灼浊兹咨资姿滋淄孜紫仔籽滓子自渍字鬃棕踪宗综总纵邹走奏揍租足卒族祖诅阻组钻纂嘴醉最罪尊遵昨左佐柞做作坐座',//后面有5个字符为空
...
var fhData = [
				'　、。·ˉˇ¨〃々—～‖…‘’“”〔〕〈〉《》「」『』〖〗【】±×÷∶∧∨∑∏∪∩∈∷√⊥∥∠⌒⊙∫∮≡≌≈∽∝≠≮≯≤≥∞∵∴♂♀°′″℃＄¤￠￡‰§№☆★○●◎◇◆□■△▲※→←↑↓〓'
				,'ⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹ⒈⒉⒊２⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇①②③④⑤⑥⑦⑧⑨⑩　　㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫ'
				,'！＂＃￥％＆＇（）＊＋，－．／０１２３４５６７８９：；＜＝＞？＠ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ［＼］＾＿｀ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ｛｜｝'
				,'ぁあぃいぅうぇえぉおかがきぎくぐけげこごさざしじすずせぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃやゅゆょよらりるれろゎわゐゑをん'
				,'ァアィイゥウェエォオカガキギクグケゲコゴサザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶ'
				,'ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ        αβγδεζηθικλμνξοπρστυφχψω       ︵︶︹︺︿﹀︽︾﹁﹂﹃﹄︻︼︷︸︱︳︴'
				,'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ               абвгдеёжзийклмнопрстуфхцчшщъыьэюя'
				,'āáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜüêɑńňɡ    ㄅㄆㄇㄈㄉㄊㄋㄌㄍㄎㄏㄐㄑㄒㄓㄔㄕㄖㄗㄘㄙㄚㄛㄜㄝㄞㄟㄠㄡㄢㄣㄤㄥㄦㄧㄨㄩ'
				,'   ─━│┃┄┅┆┇┈┉┊┋┌┍┎┏┐┑┒┓└┕┖┗┘┙┚┛├┝┞┟┠┡┢┣┤┥┦┧┨┩┪┫┬┭┮┯┰┱┲┳┴┵┶┷┸┹┺┻┼┽┾┿╀╁╂╃╄╅╆╇╈╉╊╋'
			];

		
//** escape()编码后的值：   u4e00 u9fa5 uf900 ufa2d
//** tmp.charCodeAt(0)的值：19968 40869 63744 64045
//** /^[\u4e00-\u9fa5\uf900-\ufa2d]$/.test(tmp);

//str=加密字符串
//isURI=是否进行URI处理 true|false
function URIencode(vStr,isURI){
	var vGet = new String();
	
	for (var i = 0; i < vStr.length; i ++) {
		var tmp = vStr.charAt(i);
		
		var code = vStr.charCodeAt(i);
        if(code >= 19968 && code <= 40869){
			...
			for (var area = 0; area < fhData.length; area ++) {
				var place = fhData[area].indexOf(tmp) + 1;
				if (place) {
					area += 1;
					vGet += '%' + getUCase((0xA0 + area).toString(16));
					vGet += '%' + getUCase((0xA0 + place).toString(16));
...
function encodeToGB2312(str) {
	return URIencode(str,false);
}

function encodeToUTF8(str){
	return encodeURI(str);
}
function decodeFromUTF8(str){
	return decodeURI(str);
}
function fun_escape(str){
	return escape(str);
}
function fun_encodeURI(str){
	return encodeURI(str);
}
function fun_encodeURIComponent(str){
