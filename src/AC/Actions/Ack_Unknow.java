package AC.Actions;

import AC.Utils.WR;
import java.io.PrintStream;
import java.net.DatagramPacket;
import java.net.DatagramSocket;
import java.net.InetAddress;

public class Ack_Unknow
{
  public static void Ack(String ip, int port, byte[] Req_Data)
  {
    System.out.println("收到" + ip + ":" + port + ":" + "发来的报文：" + 
      WR.Getbyte2HexString(Req_Data));

    byte[] Ack_Data_Quit = new byte[6];

    for (int i = 0; i < Ack_Data_Quit.length; i++) {
      Ack_Data_Quit[i] = 1;
    }

    System.out.println("准备回复报文给：" + ip + ":" + port + ":" + 
      WR.Getbyte2HexString(Ack_Data_Quit));

    DatagramSocket scoket_Unknow_ACK = null;
    try {
      scoket_Unknow_ACK = new DatagramSocket();
      DatagramPacket data = new DatagramPacket(Ack_Data_Quit, 
        Ack_Data_Quit.length, InetAddress.getByName(ip), port);
      scoket_Unknow_ACK.send(data);
    }
    catch (Exception e)
    {
      e.printStackTrace();
    } finally {
      scoket_Unknow_ACK.close();
    }
    WR.space();
  }
}

/* Location:           C:\Users\Thinkpad\Desktop\Tool\jd-gui\jd-gui\spring-ops-3.2.4.RELEASE.jar
 * Qualified Name:     AC.Actions.Ack_Unknow
 * JD-Core Version:    0.6.2
 */